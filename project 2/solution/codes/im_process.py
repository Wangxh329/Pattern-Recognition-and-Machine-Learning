...
def nms(xyxys, overlap_thresh):
 ...
 ######################
	...
	# generate coordinates and score lists
	x1 = xyxys[:, 0]
	y1 = xyxys[:, 1]
	x2 = xyxys[:, 2]
	y2 = xyxys[:, 3]
	...
	while order.size > 0:
		i = order[0]
		keep.append(i)
		# get overlap rectangle coordinates
		...
		inter = w * h
		#IOU: (overlap area / (area1 + area2 - overlap area))
		ovr = inter / (areas[i] + areas[order[1:]] - inter)

		#finds all rectangles under overlap threshold 
		inds = np.where(ovr <= overlap_thresh)[0]
		#update order. Note the plus one is because the first rectangle we have added to keep
		order = order[inds + 1]
	return xyxys[keep]
